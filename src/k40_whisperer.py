...
class Application(Frame):
    def __init__(self, master, opts=None, args=None):
        ...
        #self.write_egv_to_file('', 'tmp.txt')
...
##        self.unsharp_t.set("3")
        ...
        self.Label_Reng_pow_u = Label(self.master,textvariable=self.funits, anchor=W)
        self.Entry_Reng_pow   = Entry(self.master,width="15")
        self.Entry_Reng_pow.configure(textvariable=self.Reng_pow,justify='center',fg="red")
        self.Reng_pow.trace_variable("w", self.Entry_Reng_pow_Callback)
        self.NormalColor =  self.Entry_Reng_pow.cget('bg')

        self.Label_Veng_pow_u = Label(self.master,textvariable=self.funits, anchor=W)
        self.Entry_Veng_pow   = Entry(self.master,width="15")
        self.Entry_Veng_pow.configure(textvariable=self.Veng_pow,justify='center',fg="red")
        self.Veng_pow.trace_variable("w", self.Entry_Veng_pow_Callback)
        self.NormalColor =  self.Entry_Veng_pow.cget('bg')

        self.Label_Vcut_pow_u = Label(self.master,textvariable=self.funits, anchor=W)
        self.Entry_Vcut_pow   = Entry(self.master,width="15")
        self.Entry_Vcut_pow.configure(textvariable=self.Vcut_pow,justify='center',fg="red")
        self.Vcut_pow.trace_variable("w", self.Entry_Vcut_pow_Callback)
        self.NormalColor =  self.Entry_Vcut_pow.cget('bg')
...
##        self.refreshTime()
    ...
    def refreshTime(self):
        ...
        if self.k40 != None:
           rapid_feed = self.k40.linear_rapid / 25.4   # 100 mm/s move feed to be confirmed
    ...
    #############################
    def Entry_Reng_pow_Check(self):
        try:
            value = float(self.Reng_pow.get())
            if  value < 0:
                self.statusMessage.set(" power should be greater than or equal to %f " %(0))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.refreshTime()
        return 0         # Value is a valid number
    def Entry_Reng_pow_Callback(self, varName, index, mode):
        self.entry_set(self.Entry_Reng_pow, self.Entry_Reng_pow_Check(), new=1)        
    #############################
    def Entry_Veng_pow_Check(self):
        try:
            value = float(self.Veng_pow.get())
            if  value < 0:
                self.statusMessage.set(" power should be greater than or equal to %f " %(0))
                return 2 # Value is invalid number
        except:
        ...
        self.refreshTime()
        return 0         # Value is a valid number
    def Entry_Veng_pow_Callback(self, varName, index, mode):
        self.entry_set(self.Entry_Veng_pow, self.Entry_Veng_pow_Check(), new=1)        
    #############################
    def Entry_Vcut_pow_Check(self):
        try:
            value = float(self.Vcut_pow.get())
            if  value < 0:
                self.statusMessage.set(" power should be greater than or equal to %f " %(0))
                return 2 # Value is invalid number
        except:
        ...
        self.refreshTime()
        return 0         # Value is a valid number
    def Entry_Vcut_pow_Callback(self, varName, index, mode):
        self.entry_set(self.Entry_Vcut_pow, self.Entry_Vcut_pow_Check(), new=1)        
    #############################
...
##        self.entry_set(self.Entry_Unsharp_Threshold, self.Entry_Unsharp_Threshold_Check(), new=1)
    ...
    def menu_File_Open_Design(self,event=None):
        ...
        self.fileload(fileselect)
            

    def fileload(self, fileselect, units=None):
    ...
    def menu_File_save_EGV(self,operation_type=None,default_name="out.EGV"):
        ...
        if filename != '' and filename != ():
            ...
            self.write_egv_to_file(data, filename)
...
##                    image_temp = image_temp.filter(filter)
    ...
    def Open_G_Code(self,filename):
        ...
        ecoords= g_rip.generate_laser_paths(
              g_rip.g_code_data,
              spindle_power_scale=float(self.gcode_import_spindle_power_scale.get())
             )
    ...
    def Open_DXF(self,filemname, units=None):
        self.resetPath()
...
##            fout.close
    ...
    def Get_Design_Bounds(self):
        ...
        #print(f'   Get_Design_Bounds: x:{[xmin,xmax]}  y:{[ymin,ymax]}')
    ...
    def scale_offset_flipy_vector_coords(self, coords, startx, starty, flipy):
        if coords == None: return
        if coords[0] == None: return

        # flipy = False
        #    scale and offset such that [startx,starty] --> [0,0]
        # flipy = True
        #    scale and offset such that [startx,starty] --> [0,0]
        #    mirror about x-axis
        ...
        for i in range(len(coords)):
            coords_scale.append(coords[i][:])
            x = coords_scale[i][0] - startx
            y = coords_scale[i][1] - starty
            coords_scale[i][0] = x * Xscale
            coords_scale[i][1] = y * Yscale
    ...
    def send_data(self, operation_type=None):
        ...
        elif self.k40.dialect == 'ecoord':
            data = self.prep_ecoord_data(operation_type)
 
            gc, segtime = self.k40.ecoord_to_gcode(data)
            #print(gc)
            if self.preview(gc) == "send":
               self.send_machine_data(data, 1)

        else:
            self.statusMessage.set(f"Laser Cutter has unknown dialect: {self.k40.dialect}")
            self.statusbar.configure( bg = 'red' ) 
    ...
    def preview(self, data):
        ...
        w.grab_set()
        #w.resizable(0,0)
        w.title('GCode preview')
        return_value =  StringVar()
        return_value.set("none")

        def Close_Click():
            return_value.set("send")
            w.destroy()
        ...
        text = Text(tf, width="80", height="20", yscrollcommand = sb.set, bg='white')
        for line in data:
    ...
    def prep_ecoord_data(self, operation_type=None):
        ...
        # mirror, scale, offset and flip for all modes
        for i in range(len(cutcoords)):
            if self.mirror.get() or self.rotate.get():
                cutcoords[i] = self.mirror_rotate_vector_coords(cutcoords[i])
            cutcoords[i] = self.scale_offset_flipy_vector_coords(cutcoords[i], startx, starty, self.k40.flipy)

        # chain them all together
        data=[]
        for i in range(len(cutcoords)):
            if cutcoords[i] != None and len(cutcoords[i]) > 0:
                for k in range(passes[i]):
                    data.extend(cutcoords[i])

        return data

    def prep_egv_data(self, operation_type=None):
        num_passes=0
        try:
            Raster_Eng_data=[]
            Vector_Eng_data=[]
...
##                    plt.plot(X,Y)
    ...
    def send_machine_data(self,data,num_passes=1):        
        return self.send_egv_data(data,num_passes=1)
    ...
    def write_egv_to_file(self, data, fname):
        print(f'write_egv_to_file   fname={fname}')
        #fname = "tmp.egv"
        if len(data) == 0:
            raise Exception("No data available to write to file.")
        #try:
        #    #fout = open(fname,'w')
        #    fout = open(fname,'w', buffering=1)
        #except:
    ...
    def Master_Configure(self, event, update=0):
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            if True:                
                ...
                x_pow_L=x_units_L+w_units+20-5
                x_pow_u=x_pow_L+w_units+20-5
                ...
                if self.GcodeData.ecoords == []:
                    ...
                    if False:
                        ...
                        self.Entry_Vcut_pow.place   (x=x_pow_L, y=Yloc, width=w_entry, height=23)
                        self.Label_Vcut_pct.place(x=x_pow_u, y=Yloc, width=w_units/3, height=23)
                        Y_Vcut=Yloc
                        ...
                        self.Entry_Veng_pow.place(  x=x_pow_L, y=Yloc, width=w_entry, height=23)
                        self.Label_Veng_pct.place(x=x_pow_u, y=Yloc, width=w_units/3, height=23)
                        Y_Veng=Yloc
                        ...
                        self.Entry_Reng_pow.place(  x=x_pow_L, y=Yloc, width=w_entry, height=23)
                        self.Label_Reng_pct.place(x=x_pow_u, y=Yloc, width=w_units/3, height=23)
                        Y_Reng=Yloc
                    else:
                        #  feed and power values table
                        Yloc=Yloc-30
                        self.Vcut_Button.place      (x=12, y=Yloc, width=100, height=23)
                        self.Entry_Vcut_feed.place  (x=x_entry_L, y=Yloc, width=w_entry, height=23)
                        self.Entry_Vcut_pow.place   (x=x_units_L, y=Yloc, width=w_entry, height=23)
                        Y_Vcut=Yloc

                        Yloc=Yloc-30
                        self.Veng_Button.place  (x=12, y=Yloc, width=100, height=23)
                        self.Entry_Veng_feed.place(  x=x_entry_L, y=Yloc, width=w_entry, height=23)
                        self.Entry_Veng_pow.place(  x=x_units_L, y=Yloc, width=w_entry, height=23)
                        Y_Veng=Yloc
                        
                        Yloc=Yloc-30
                        self.Reng_Button.place  (x=12, y=Yloc, width=100, height=23)
                        self.Entry_Reng_feed.place(  x=x_entry_L, y=Yloc, width=w_entry, height=23)
                        self.Entry_Reng_pow.place(  x=x_units_L, y=Yloc, width=w_entry, height=23)
                        Y_Reng=Yloc

                        Yloc=Yloc-25
                        self.Label_Reng_feed_u.place(x=x_entry_L, y=Yloc, width=w_units, height=23)
                        self.Label_Reng_pct.place(x=x_units_L, y=Yloc, width=w_units, height=23)
...
##                            plot_im = plot_im.filter(filter)
    ...
    ################################################################################
    # Moves head around inside the bounding box of the design.
    # Its a temporary move because the laserX and laserY params
    # do not change.
    # laserX, laserY is the position of the design UL (or UR is HomeUR flag is
    # set) on the lsaer bed.
...
if __name__ == "__main__":
    ...
    app = Application(root, opts, args)
